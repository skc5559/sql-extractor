...
import com.test.ColumnImpact.TColumn;
...
public class ColumnAnalyze
{
	...
	private void impactSQL( TGSqlParser sqlparser )
	{
		...
		if ( ret != 0 )
		{
			errorMessage = sqlparser.getErrormessage( ) + "+++" + sqlparser.sqlfilename;
		}
		else
		{
			try {
			...
			{
				buffer.append( "SCHEMA_NAME|TABLE_NAME|ATTRIBUTES_NAME|COUNT\n" );
				...
				while ( tableIter.hasNext( ) )
				{
					...
					{
						buffer.append(schemaAndTable[0])
						       .append("|")
						       .append(schemaAndTable[1]);
					}
					else 
					{
						buffer.append("")
...
//			}
	...
	{
		Map<String, List<ESqlClause>> columns = tableColumns.get( tableName );
		List<ESqlClause> clauses = null;
		if(column.columnName!=null)
		{
			clauses = columns.get( column.columnName.toUpperCase( ) );
		}
	...
	public static void main( String[] args )
	{
		...
		File failedDir = new File(args[2]);
		...
		{
			System.out.println( failedDir + " is not a valid directory." );
			return;
		}
		...
		for ( int i = 0; i < files.length; i++ )
		{
			if ( files[i].isFile( ) )
			{
				...
				{
					System.out.println( "Failure Message" + analyze.getErrorMessage( ) );
					try 
					{
						
						Files.move(files[i].toPath(), failedDir.toPath().resolve(files[i].getName()), StandardCopyOption.REPLACE_EXISTING);
